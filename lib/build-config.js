 ...
 * По умолчанию только IPv4 — так конфиг не требует ручной чистки адресов.
 */
export const IP_VERSIONS = ['both', 'ipv4', 'ipv6'];
export const DEF_IP_VERSION = 'ipv4';

/**
 * queryStrategy у dns — единственный рычаг версии IP, который Xray 26.3.27
 * действительно понимает. Проверено по бинарнику: UseIP, UseIPv4 и UseIPv6
 * в нём есть, а ForceIPv4 / ForceIPv6 / UseIPv4v6 отсутствуют совсем, то есть
 * были бы проглочены молча. Сам `xray run -test` тут не судья: он отвечает
 * Configuration OK и на заведомо несуществующее значение.
 */
const QUERY_STRATEGY = { both: 'UseIP', ipv4: 'UseIPv4', ipv6: 'UseIPv6' };
...
export function buildConfig({
  ...
  cfg.dns = { servers: dnsServers, queryStrategy: QUERY_STRATEGY[ipVersion] };
  if (inbounds) {
    cfg.inbounds = [
      {
        tag: 'socks-in',
