...
/**
 * Версия IP внутри туннеля: обе, только IPv4 или только IPv6.
 *
 * По умолчанию — обе. Это в точности тот профиль, который проходил полевые
 * прогоны на мобильной сети, и трогать его по своей инициативе нельзя:
 * любое сужение versions меняет то, что проверено в бою. Выбор отдан
 * пользователю в настройках.
 */
export const IP_VERSIONS = ['both', 'ipv4', 'ipv6'];
export const DEF_IP_VERSION = 'both';

/**
 * Допустимые значения dns.queryStrategy. Проверено по бинарнику Xray 26.3.27:
 * UseIP, UseIPv4 и UseIPv6 в нём есть, а ForceIPv4 / ForceIPv6 / UseIPv4v6
 * отсутствуют совсем — их Xray проглотил бы молча. `xray run -test` тут не
 * судья: он отвечает Configuration OK и на заведомо несуществующее значение.
 *
 * По умолчанию поле в конфиг НЕ пишется вовсе: в проверенном профиле его нет,
 * а с ним конфиг у пользователя ломался. Включается только вручную.
 */
export const QUERY_STRATEGIES = ['UseIP', 'UseIPv4', 'UseIPv6'];
export const DEF_QUERY_STRATEGY = null;

const ALLOWED_IPS_BY_VERSION = {
  both: ['0.0.0.0/0', '::/0'],
  ipv4: ['0.0.0.0/0'],
  ipv6: ['::/0'],
};
...
/** Семейство адреса: 'ipv4', 'ipv6' или null, если это вообще не адрес. */
export function addressFamily(value) {
  const a = String(value ?? '').trim().split('/')[0].replace(/^\[|\]$/g, '');
  if (isIPv4(a)) return 'ipv4';
  if (isIPv6(a)) return 'ipv6';
  return null;
}

/**
 * Оставляет из списка только адреса нужной версии.
 * Значения, которые адресами не являются (имя хоста, localhost, URL DoH),
 * сохраняются всегда — отфильтровать их по версии невозможно, а выбрасывать
 * молча значит испортить пользователю настройку.
 */
export function filterByIpVersion(list, ipVersion) {
  const items = (Array.isArray(list) ? list : [list]).filter((v) => v != null && v !== '');
  if (ipVersion === 'both') return [...items];
  return items.filter((v) => {
    const fam = addressFamily(v);
    return fam === null || fam === ipVersion;
  });
}
...
export function buildConfig({
  ...
  dns = DEF_DNS,
  ipVersion = DEF_IP_VERSION,
  queryStrategy = DEF_QUERY_STRATEGY,
  ...
  if (!IP_VERSIONS.includes(ipVersion)) {
    throw new Error(`недопустимый ipVersion: ${JSON.stringify(ipVersion)} (ожидалось ${IP_VERSIONS.join(', ')})`);
  }
  const allAddresses = (Array.isArray(address) ? address : [address])
    .map(normAddress)
    .filter(Boolean);
  if (allAddresses.length === 0) throw new Error('не задан ни один адрес интерфейса');
  const addresses = filterByIpVersion(allAddresses, ipVersion);
  if (addresses.length === 0) {
    const want = ipVersion === 'ipv4' ? 'IPv4' : 'IPv6';
    throw new Error(`выбран режим «только ${want}», но среди адресов интерфейса нет ни одного ${want}: ${allAddresses.join(', ')}`);
  }
  ...
  const warp = {
    ...
    settings: {
      ...
      peers: [{
        ...
        endpoint: ep,
        keepAlive: keepAlive,
        allowedIPs: [...ALLOWED_IPS_BY_VERSION[ipVersion]],
      }],
    },
  ...
  cfg.log = { loglevel };
  const dnsList = Array.isArray(dns) ? dns : String(dns).split(',').map((s) => s.trim()).filter(Boolean);
  const dnsServers = filterByIpVersion(dnsList, ipVersion);
  if (dnsServers.length === 0) {
    const want = ipVersion === 'ipv4' ? 'IPv4' : 'IPv6';
    throw new Error(`выбран режим «только ${want}», но среди DNS-серверов нет ни одного ${want}: ${dnsList.join(', ')}`);
  }
  cfg.dns = { servers: dnsServers };
  if (queryStrategy) {
    if (!QUERY_STRATEGIES.includes(queryStrategy)) {
      throw new Error(`недопустимый queryStrategy: ${JSON.stringify(queryStrategy)} (Xray понимает только ${QUERY_STRATEGIES.join(', ')})`);
    }
    cfg.dns.queryStrategy = queryStrategy;
  }
  if (inbounds) {
    cfg.inbounds = [
      {
